...
- A finding carrying `value[:12]` or `value[::-1]` — a substring test cannot see either, and a reader recovers both instantly. Replaced by `_carries`: does any **eight-character window**, forwards or backwards, appear.
- **`_describe_changes` had no test of its own at all.** The rotation proof exercises it through one shape, with fixture values *shorter than every real coordinate* — so a `digest()` leaking only above a length threshold passed, as did a reversing `digest()`, as did the `appeared`/`removed` branches the both-sides fixture never reaches. Three leaks, all green. It now has `test_the_drift_report_never_carries_a_value_in_any_recoverable_form`, driven directly through all three shapes with an over-long opaque value.

**And a helper's parameter is a publication channel.** `_dotenv_use` took file *text*; pytest renders a frame's arguments, and `ast.parse` raises on any malformed module — so a package file that both failed to parse and carried a value would have printed itself in full. It takes a `Path` now and reads inside.

**What is deliberately NOT chased, so the next reader does not re-open it.** Five mutations survive and are refused rather than missed: four narrow the scan's *scope* (a length floor, a dropped section, a swallowed `SyntaxError`, a `break` after the first finding), and one deletes the no-print assertion itself. The first four are the scan's scope, which is **#243**'s entire subject and where they are routed — fixing them here would be doing that story badly, twice. The fifth is infinite regress: a guard on the guard on the guard has no fixed point, and the register is where that risk is carried instead. The `_carries` window is eight characters, which is a judgement and not a law; a six-character leak would pass, and chasing that is the whack-a-mole this epic exists to refuse.
