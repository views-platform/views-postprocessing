...
Reproduced 2026-08-17 against a tracked-files-only worktree (`git worktree add --detach`, which contains exactly what `actions/checkout` produces): `1 failed, 37 passed, 1 skipped`, the failure being `FileNotFoundError: .../gaul0_code.parquet`. After re-gating on the seven parquets themselves: `35 passed, 4 skipped`, no failures. views-datafactory is now fetched in `run_pytest.yml` and declared `ci_checkout=True`. Two checks moved from skipped to running in the gate — this entry's `TestReleaseGate::test_land_gaul_commit_is_in_a_release_tag`, and C-30's exclusion-manifest tripwire. The producer-comparison half still skips, honestly, and still needs the parquets published somewhere fetchable.
