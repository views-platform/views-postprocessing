...
class PriogridCountryMapper:
    ...
    def __init__(self, use_disk_cache=True, cache_dir=None, cache_ttl=None):
        ...
        if use_disk_cache:
            ...
            self._disk_country_cache = Memory(location=self.cache_dir / "country", verbose=0)
            self._disk_admin1_cache = Memory(location=self.cache_dir / "admin1", verbose=0)
            self._disk_admin2_cache = Memory(location=self.cache_dir / "admin2", verbose=0)
            ...
            if cache_ttl:
                self._country_cache = TTLCache(maxsize=COUNTRY_CACHE_MAXSIZE, ttl=cache_ttl)
                self._gid_cache = TTLCache(maxsize=GID_CACHE_MAXSIZE, ttl=cache_ttl)
                self._gids_for_country_cache = TTLCache(maxsize=GIDS_FOR_COUNTRY_CACHE_MAXSIZE, ttl=cache_ttl)
                self._admin1_cache = TTLCache(maxsize=ADMIN1_CACHE_MAXSIZE, ttl=cache_ttl)
                self._admin2_cache = TTLCache(maxsize=ADMIN2_CACHE_MAXSIZE, ttl=cache_ttl)
            else:
                self._country_cache = LRUCache(maxsize=COUNTRY_CACHE_MAXSIZE)
                self._gid_cache = LRUCache(maxsize=GID_CACHE_MAXSIZE)
                self._gids_for_country_cache = LRUCache(maxsize=GIDS_FOR_COUNTRY_CACHE_MAXSIZE)
    ...
    def get_cache_stats(self):
        ...
        if self.use_disk_cache:
            ...
            total_size = 0
            for cache_dir in [self.cache_dir / d for d in ["country", "admin1", "admin2", "gid"]]:
                if cache_dir.exists():
            ...
            if hasattr(self._country_cache, 'currsize'):
                stats["country_cache_hit_rate"] = f"{self._country_cache.hits}/{self._country_cache.hits + self._country_cache.misses}"
                stats["gid_cache_hit_rate"] = f"{self._gid_cache.hits}/{self._gid_cache.hits + self._gid_cache.misses}"
    ...
    def batch_country_mapping_parallel(self, gid_list, batch_size=1000):
        ...
        # Split into batches
        batches = [gid_list[i:i + batch_size] for i in range(0, len(gid_list), batch_size)]
        ...
        if self.use_disk_cache:
            ...
            def _find_country_for_gid_impl(gid):
                ...
                if hasattr(self.countries_gdf, 'sindex'):
                    possible_countries_idx = list(self.countries_gdf.sindex.intersection(grid_geometry.bounds))
                    candidate_countries = self.countries_gdf.iloc[possible_countries_idx]
                    candidate_countries = candidate_countries[candidate_countries.intersects(grid_geometry)]
                else:
                    candidate_countries = self.countries_gdf[self.countries_gdf.intersects(grid_geometry)]
                ...
                if overlaps:
                    result = {
                        ...
                        "overlap_ratio": float(overlaps[0]["overlap_ratio"]) if overlaps else 0.0,
                        "method": method_used,
                    }
    ...
    def batch_country_mapping(self, gid_list):
        ...
        if cache_hits > 0:
            logger.info(f"Cache hits: {cache_hits}/{len(gid_list)} ({cache_hits/len(gid_list)*100:.1f}%)")
        ...
        if self.use_disk_cache:
            ...
            def _find_gids_for_country_impl(iso_a3):
                ...
                if len(country) == 0:
                    ...
                    if self.priogrid_sindex:
                        possible_matches_index = list(self.priogrid_sindex.intersection(country_geometry.bounds))
                        candidate_grid_cells = self.priogrid_gdf.iloc[possible_matches_index]
                        intersecting_cells = candidate_grid_cells[candidate_grid_cells.intersects(country_geometry)]
                    else:
                        intersecting_cells = self.priogrid_gdf[self.priogrid_gdf.intersects(country_geometry)]
    ...
    def visualize_grid_and_country(self, gid):
        ...
        if country_info:
            ...
            containing_country = self.countries_gdf[self.countries_gdf["ISO_A3"] == iso_a3].iloc[0]
        ...
        if self.use_disk_cache:
            ...
            def _find_admin1_for_gid_impl(gid):
                ...
                if self.admin1_sindex:
                    possible_admin1_idx = list(self.admin1_sindex.intersection(grid_geometry.bounds))
                    candidate_admin1 = self.admin1_gdf.iloc[possible_admin1_idx]
                    candidate_admin1 = candidate_admin1[candidate_admin1.intersects(grid_geometry)]
                    # Filter to only those in the same country
                    candidate_admin1 = candidate_admin1[candidate_admin1["iso3_code"] == iso_a3]
                else:
                    candidate_admin1 = country_admin1[country_admin1.intersects(grid_geometry)]
                ...
                result = {
                    "gid": int(gid),
                    "gaul1_code": int(admin1["gaul1_code"]) if admin1["gaul1_code"] is not None else None,
                ...
                if "gaul0_code" in admin1:
                    result["gaul0_code"] = int(admin1["gaul0_code"]) if admin1["gaul0_code"] is not None else None
        ...
        if self.use_disk_cache:
            ...
            def _find_admin2_for_gid_impl(gid):
                ...
                if self.admin2_sindex:
                    possible_admin2_idx = list(self.admin2_sindex.intersection(grid_geometry.bounds))
                    candidate_admin2 = self.admin2_gdf.iloc[possible_admin2_idx]
                    candidate_admin2 = candidate_admin2[candidate_admin2.intersects(grid_geometry)]
                    # Filter to only those in the same country
                    candidate_admin2 = candidate_admin2[candidate_admin2["iso3_code"] == iso_a3]
                else:
                    candidate_admin2 = country_admin2[country_admin2.intersects(grid_geometry)]
                ...
                result = {
                    "gid": int(gid),
                    "gaul2_code": int(admin2["gaul2_code"]) if admin2["gaul2_code"] is not None else None,
                ...
                if "gaul0_code" in admin2:
                    result["gaul0_code"] = int(admin2["gaul0_code"]) if admin2["gaul0_code"] is not None else None
                if "gaul0_name" in admin2:
                    result["gaul0_name"] = admin2["gaul0_name"]
                if "gaul1_code" in admin2:
                    result["gaul1_code"] = int(admin2["gaul1_code"]) if admin2["gaul1_code"] is not None else None
    ...
    def batch_admin_mapping_parallel(self, gid_list, admin_level="admin1", batch_size=1000):
        """
    ...
    def visualize_grid_and_admin(self, gid, admin_level="admin1", show_all_admins=False):
        """
        ...
        if show_all_admins:
            ...
            if admin_level == "admin1" and self.admin1_sindex:
                possible_admins_idx = list(self.admin1_sindex.intersection(grid_geometry.bounds))
                nearby_admins = self.admin1_gdf.iloc[possible_admins_idx]
            elif admin_level == "admin2" and self.admin2_sindex:
                possible_admins_idx = list(self.admin2_sindex.intersection(grid_geometry.bounds))
            ...
            nearby_admins.plot(ax=ax, color="lightgray", edgecolor="white", alpha=0.5, linewidth=0.5)
            
            # Highlight the containing admin region if found
            if admin_info:
                containing_admin = admin_gdf[admin_gdf[admin_code_col] == admin_info[admin_code_col]]
                if len(containing_admin) > 0:
                    containing_admin.plot(ax=ax, color="lightblue", edgecolor="blue", alpha=0.7, linewidth=1.5)
        else:
        ...
        legend_elements = [
            ...
            mpatches.Patch(color='lightblue', alpha=0.7, label=f'Containing {admin_level.upper()}'),
            mpatches.Patch(color='lightgray', alpha=0.5, label='Other Admin Regions' if show_all_admins else None),
        ]
    ...
    def visualize_admin_regions_for_gids(self, gid_list, admin_level="admin1", show_grid_cells=True, 
                                        color_by_country=True, figsize=(15, 10)):
        """
        ...
        if color_by_country:
            ...
            admin_regions.plot(ax=ax, color="lightblue", edgecolor="black", alpha=0.7, linewidth=1)
            legend_elements = [
                mpatches.Patch(color='lightblue', alpha=0.7, label=f'{admin_level.upper()} Regions')
            ]
        
        # Plot grid cells if requested
        if show_grid_cells:
            grid_cells.plot(ax=ax, color="red", alpha=0.8, edgecolor="darkred", linewidth=1.5)
        ...
        if show_grid_cells:
            legend_elements.insert(0, mpatches.Patch(color='red', alpha=0.8, label='PRIO-GRID Cells'))
    ...
    def find_country_by_iso_a3(self, iso_a3):
        ...
        if not isinstance(iso_a3, str) or len(iso_a3) != 3:
            logger.warning(f"Invalid ISO A3 code format: {iso_a3}. Expected 3-character string.")
            return None
        ...
        if self.use_disk_cache:
            ...
            if len(country_rows) == 0:
                logger.warning(f"Country with ISO A3 code '{iso_a3}' not found in Natural Earth dataset")
    ...
    def find_multiple_countries_by_iso_a3(self, iso_a3_list):
        ...
        if not isinstance(iso_a3_list, (list, tuple, set)):
            raise ValueError("iso_a3_list must be a list, tuple, or set of ISO A3 codes")
        ...
        for iso_a3 in iso_a3_list:
            ...
            if cache_key in self._country_cache:
                cached_value = self._country_cache[cache_key]
                results[iso_a3.upper()] = cached_value.copy() if cached_value is not None else None
                cache_hits += 1
            else:
        ...
        if cache_hits > 0:
            logger.info(f"Cache hits: {cache_hits}/{len(iso_a3_list)} ({cache_hits/len(iso_a3_list)*100:.1f}%)")
    ...
    def search_countries_by_name(self, name_pattern, exact_match=False, case_sensitive=False):
        """
        ...
        for _, country in matching_countries.iterrows():
            ...
            key_fields = ["CONTINENT", "REGION_UN", "SUBREGION", "POP_EST", "GDP_MD", "INCOME_GRP"]
    ...
    def enrich_dataframe_with_pg_info(self, df, pg_id_col="priogrid_id", time_id_col="month_id", 
                            include_country=True, include_admin1=True, include_admin2=True, 
                            include_pg_info=True, country_cols=None, admin1_cols=None, admin2_cols=None, pg_cols=None,
                            batch_size=1000, use_multiprocessing=True, show_progress=True, only_metadata=True):
        """
        ...
        logger.info(f"Starting enrichment of {total_ids} unique PRIO-GRID IDs in {total_batches} batches")
        logger.info(f"Parameters: country={include_country}, admin1={include_admin1}, admin2={include_admin2}, pg_info={include_pg_info}")
        ...
        if show_progress:
            try:
                ...
                pbar = tqdm(total=total_ids, desc="Processing PRIO-GRID IDs", unit="ids")
            except ImportError:
                logger.warning("tqdm not installed. Progress bar will not be shown. Install with: pip install tqdm")
                show_progress = False
        ...
        if use_multiprocessing and total_ids > batch_size:
            ...
            # Split into batches
            batches = [unique_pg_ids[i:i+batch_size] for i in range(0, total_ids, batch_size)]
            
            # Process batches in parallel using threads
            with ThreadPoolExecutor(max_workers=self._max_workers) as executor:
                # Submit all batches for processing
                future_to_batch = {
                    executor.submit(self._process_pg_batch, batch_ids, 
                                    include_country, include_admin1, include_admin2, 
                                    include_pg_info, country_cols, admin1_cols, admin2_cols, pg_cols): batch_idx
                ...
                for future in as_completed(future_to_batch):
                    ...
                    try:
                        ...
                        if batch_idx % max(5, total_batches // 10) == 0 or batch_idx == total_batches - 1:
                            progress_pct = (processed_count / total_ids) * 100
                            logger.info(f"Progress: {processed_count}/{total_ids} ({progress_pct:.1f}%) - Batch {batch_idx + 1}/{total_batches}")
            ...
            for i in range(0, total_ids, batch_size):
                ...
                try:
                    batch_result = self._process_pg_batch(batch_ids, 
                                                include_country, include_admin1, include_admin2, 
                                                include_pg_info, country_cols, admin1_cols, admin2_cols, pg_cols)
                    all_pg_data.update(batch_result)
                    ...
                    # Log progress
                    progress_pct = (processed_count / total_ids) * 100
                    logger.info(f"Completed batch {batch_num}/{total_batches} - {progress_pct:.1f}% complete")
        ...
        logger.info(f"Enrichment complete. Added {len(result_df.columns) - len(df.columns)} new columns")
        return result_df
    
    def _process_pg_batch(self, pg_ids, include_country, include_admin1, include_admin2, 
                        include_pg_info, country_cols, admin1_cols, admin2_cols, pg_cols):
    ...
    def enrich_dataframe_with_country_info(self, df, iso_a3_col="iso_a3",
                                     include_admin1=True, include_admin2=True,
                                     country_cols=None, admin1_cols=None, admin2_cols=None,
                                     batch_size=1000, use_multiprocessing=True, show_progress=True):
        """
        ...
        logger.info(f"Starting enrichment of {total_codes} unique ISO A3 codes in {total_batches} batches")
        ...
        if show_progress:
            try:
                from tqdm import tqdm
                pbar = tqdm(total=total_codes, desc="Processing ISO A3 codes", unit="codes")
            except ImportError:
                logger.warning("tqdm not installed. Progress bar will not be shown. Install with: pip install tqdm")
                show_progress = False
        ...
        if use_multiprocessing and total_codes > batch_size:
            ...
            # Split into batches
            batches = [unique_iso_a3[i:i+batch_size] for i in range(0, total_codes, batch_size)]
            
            # Process batches in parallel using threads
            with ThreadPoolExecutor(max_workers=self._max_workers) as executor:
                # Submit all batches for processing
                future_to_batch = {
                    executor.submit(self._process_country_batch, batch_codes, **process_params): batch_idx
                ...
                for future in as_completed(future_to_batch):
                    ...
                    try:
                        ...
                        if batch_idx % max(5, total_batches // 10) == 0 or batch_idx == total_batches - 1:
                            ...
                            logger.info(f"Progress: {processed_count}/{total_codes} ({progress_pct:.1f}%) - Batch {batch_idx + 1}/{total_batches}")
            ...
            for i in range(0, total_codes, batch_size):
                ...
                logger.info(f"Processing batch {batch_num}/{total_batches} ({len(batch_codes)} codes)")
                
                try:
                    batch_result = self._process_country_batch(batch_codes, **process_params)
                    all_country_data.update(batch_result)
    ...
    def _process_country_batch(self, iso_a3_codes, include_admin1, include_admin2,
                            country_cols, admin1_cols, admin2_cols):
                    ...
                    for _, admin1 in admin1_regions.iterrows():
                        ...
                        if admin1_cols is not None:
                            ...
                            for key, value in admin1.items():
                                if key != "iso3_code":  # Skip iso3_code as it's already known
    ...
    def add_country_info_to_dataframe(self, df, iso_a3_col="iso_a3", 
        ...
        # Get unique ISO A3 codes to process
        unique_iso_a3 = df[iso_a3_col].unique()
        logger.info(f"Processing {len(unique_iso_a3)} unique ISO A3 codes for country names")
        ...
        # Add additional columns if requested
        for col in iso_to_additional:
            result_df[f"country_{col}"] = result_df[iso_a3_col].map(iso_to_additional[col])
